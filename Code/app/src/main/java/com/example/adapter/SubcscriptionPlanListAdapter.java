...
import com.example.streamingapp.SelectSubscriptionPlanActivity;

import java.util.ArrayList;
import java.util.Random;

public class SubcscriptionPlanListAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public SubcscriptionPlanListAdapter(Context context, ArrayList<ItemPlan> dataList, ArrayList<String> strColor) {
    ...
    public void onBindViewHolder(@NonNull final RecyclerView.ViewHolder viewHolder, final int position) {
        ...
        holder.imgSelect.setOnClickListener(v -> {
            Intent intent = new Intent(mContext, SelectSubscriptionPlanActivity.class);
            ...
            ((SubscriptionPlanActivity) mContext).finish();
        });
        ...
        if (row_index > -1) {
            if (row_index == position) {
                ...
                if (position == dataList.size() - 1)
                    ((SubscriptionPlanActivity) mContext).showPromoCode(false);
                else
                    ((SubscriptionPlanActivity) mContext).showPromoCode(false);
