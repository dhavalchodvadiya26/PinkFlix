...
import com.example.adapter.ShowEpisodeListAdapter;
import com.example.adapter.HomeShowVideoListAdapter;
import com.example.adapter.SeasonVideoListAdapter;
...
import com.example.itemmodels.ItemEpisode;
...
public class ShowVideoDetailsActivity extends AppCompatActivity {
    ...
    HomeShowVideoListAdapter homeShowAdapter;
    String Id;
    ItemMovie itemMovie;
    String rental_plan = "";
    ...
    SeasonVideoListAdapter seasonAdapter;
    ShowEpisodeListAdapter episodeAdapter;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        rvRelated.setLayoutManager(new LinearLayoutManager(ShowVideoDetailsActivity.this, LinearLayoutManager.HORIZONTAL, false));
        rvRelated.setFocusable(false);
        rvRelated.setNestedScrollingEnabled(false);

        rvEpisode.setHasFixedSize(true);
        rvEpisode.setLayoutManager(new LinearLayoutManager(ShowVideoDetailsActivity.this, LinearLayoutManager.VERTICAL, false));
        rvEpisode.setFocusable(false);
        rvEpisode.setNestedScrollingEnabled(false);

        webView.setBackgroundColor(Color.TRANSPARENT);
        webView.getSettings().setJavaScriptEnabled(true);
        if (NetworkUtils.isConnected(ShowVideoDetailsActivity.this)) {
            getDetails();
        } else {
    ...
    private void displayData() {
        ...
        if (!mListItemRelated.isEmpty()) {
            homeShowAdapter = new HomeShowVideoListAdapter(ShowVideoDetailsActivity.this, mListItemRelated, false);
            ...
            homeShowAdapter.setOnItemClickListener(new RvOnClickListener() {
                ...
                public void onItemClick(int position) {
                    ...
                    Intent intent = new Intent(ShowVideoDetailsActivity.this, ShowVideoDetailsActivity.class);
        ...
        if (!mListSeason.isEmpty()) {
            seasonAdapter = new SeasonVideoListAdapter(ShowVideoDetailsActivity.this,
    ...
    public void showToast(String msg) {
        Toast.makeText(ShowVideoDetailsActivity.this, msg, Toast.LENGTH_SHORT).show();
    }
    ...
    private void displayEpisode() {
        episodeAdapter = new ShowEpisodeListAdapter(ShowVideoDetailsActivity.this, mListItemEpisode, isPurchased);
        ...
        if (!itemEpisode.getPremium().equalsIgnoreCase("Free")) {
            if (!itemEpisode.getPremium().equalsIgnoreCase("Rental")) {
                if (mListItemEpisode.get(playPosition).isCheck_plan()) {
                    ...
                    PremiumMovieListFragment premiumContentFragment = PremiumMovieListFragment.newInstance(Id, "Shows", itemEpisode.getPremium());
    ...
    private void setPlayer(int playPosition) {
        ...
        if (itemEpisode.getEpisodeUrl().isEmpty()) {
            ...
            EmbeddedImageListFragment embeddedImageFragment = EmbeddedImageListFragment.newInstance(itemEpisode.getEpisodeUrl(), itemEpisode.getEpisodeImage(), false);
            ...
            switch (itemEpisode.getEpisodeType()) { //URL Embed
                case "Local":
                case "URL":
                    ExoVideoPlayerFragment exoVideoPlayerFragment = ExoVideoPlayerFragment.newInstance(itemEpisode.getEpisodeUrl(), false);
                    exoVideoPlayerFragment.setShowMovie(mListItemEpisode.get(playPosition));
                    ...
                    break;
                case "Embed":
                    EmbeddedImageListFragment embeddedImageFragment = EmbeddedImageListFragment.newInstance(itemEpisode.getEpisodeUrl(), itemEpisode.getEpisodeImage(), true);
                    fragmentManager.beginTransaction().replace(R.id.playerSection, embeddedImageFragment).commitAllowingStateLoss();
                    break;
    ...
    public void onBackPressed() {
        if (isFullScreen) {
            ...
            ExoVideoPlayerFragment fragment = (ExoVideoPlayerFragment) fm.findFragmentById(R.id.playerSection);
//            requestWindowFeature(Window.FEATURE_NO_TITLE);
            ...
            if (isFromNotification) {
                Intent intent = new Intent(ShowVideoDetailsActivity.this, MainActivity.class);
                intent.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
                startActivity(intent);
                finish();
            } else {
    ...
    private void setImageIfSeasonAndEpisodeNone(String imageCover) {
        EmbeddedImageListFragment embeddedImageFragment = EmbeddedImageListFragment.newInstance("", imageCover, false);
        fragmentManager.beginTransaction().replace(R.id.playerSection, embeddedImageFragment).commitAllowingStateLoss();
    }
